...
log_file_path = 'Model_{} Lora_{}.pkl'.format(MODEL, LORA)
...
data, rescale_func = utils.load_data(DEBUG)
...
def evaluate(tokenized, test_id, real_CTR):
...
def plot_log(fig_file_path, data):
    ...
    plt.hlines(0.42, 0, len(data['epoch_rec'])-1, linestyles='dashed', colors='r', label='42% Embedding')
    plt.hlines(0.33, 0, len(data['epoch_rec'])-1, linestyles='dashed', colors='g', label='33% Random Guess', alpha=0.5)
    plt.ylim(0, 0.5)
    plt.xlabel('epoch')
    plt.ylabel('accuracy')
    plt.legend()
...
_ = evaluate(tokenized_news['test'], data['test']['test_id'], data['test']['labels'])
...
class TrainingCallback(TrainerCallback):
    def on_epoch_end(self, args, state, control, **kwargs):
        ...
        test_metrics = evaluate(tokenized_news['test'], data['test']['test_id'], data['test']['labels'])
